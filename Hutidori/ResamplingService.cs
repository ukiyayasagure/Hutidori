...
namespace BitmapTool {
  ...
  public class ResamplingService {
    ...
    public BitmapByteArray Resample(BitmapByteArray input, int nWidth, int nHeight) {
        ...
        // create bitmaps
        BitmapByteArray work = new BitmapByteArray(nWidth, height);
        BitmapByteArray output = new BitmapByteArray(nWidth, nHeight);
        ...
        double wdth = 0, center = 0, weight = 0;
        double intensityA = 0, intensityR = 0, intensityG = 0, intensityB = 0;
        ...
        if (xScale < 1.0) {
            ...
            for (i = 0; i < nWidth; i++) {
                ...
                contrib[i].wsum = 0;
                center = ((i + 0.5) / xScale);
                left = (int)Math.Floor(center - wdth);
                right = (int)Math.Ceiling(center + wdth);

                for (j = 0; j <= right-left; j++) {

                    weight = filter.GetValue(j * xScale);

                    int p = left+(int)(j+wdth);

                    if ((weight == 0) || (p < 0) || (p >= width))
                        continue;

                    contrib[i].p[contrib[i].n].pixel = p;
        ...
        for (k = 0; k < height; k++) {
            ...
            for (i = 0; i < nWidth; i++) {
                ...
                intensityA = 0; intensityB = 0; intensityG = 0; intensityR = 0;
                ...
                for (j = 0; j < contrib[i].n; j++) {
                    ...
                    intensityA += (input.A[contrib[i].p[j].pixel, k] * weight);
                    intensityR += (input.R[contrib[i].p[j].pixel, k] * weight);
                    intensityG += (input.G[contrib[i].p[j].pixel, k] * weight);
                    intensityB += (input.B[contrib[i].p[j].pixel, k] * weight);
                }

                work.A[i, k] = (Byte)Math.Min(Math.Max(Math.Round(intensityA / contrib[i].wsum), Byte.MinValue), Byte.MaxValue);
                work.R[i, k] = (Byte)Math.Min(Math.Max(Math.Round(intensityR / contrib[i].wsum), Byte.MinValue), Byte.MaxValue);
                work.G[i, k] = (Byte)Math.Min(Math.Max(Math.Round(intensityG / contrib[i].wsum), Byte.MinValue), Byte.MaxValue);
                work.B[i, k] = (Byte)Math.Min(Math.Max(Math.Round(intensityB / contrib[i].wsum), Byte.MinValue), Byte.MaxValue);
            }
        ...
        if (yScale < 1.0) {
            ...
            for (i = 0; i < nHeight; i++) {
                ...
                for (j = 0; j <= right-left; j++) {
                    ...
                    weight = filter.GetValue(j * yScale);

                    int p = left + (int)(j +wdth);

                    if ((weight == 0) || (p < 0) || (p >= height))
                        continue;
        ...
        for (k = 0; k < nWidth; k++) {
            ...
            for (i = 0; i < nHeight; i++) {
                ...
                for (j = 0; j < contrib[i].n; j++) {
                    ...
                    intensityA += (work.A[k, contrib[i].p[j].pixel] * weight);
                    intensityR += (work.R[k, contrib[i].p[j].pixel] * weight);
                    intensityG += (work.G[k, contrib[i].p[j].pixel] * weight);
                    intensityB += (work.B[k, contrib[i].p[j].pixel] * weight);
                }

                output.A[k, i] = (Byte)Math.Min(Math.Max(Math.Round(intensityA / contrib[i].wsum), Byte.MinValue), Byte.MaxValue);
                output.R[k, i] = (Byte)Math.Min(Math.Max(Math.Round(intensityR / contrib[i].wsum), Byte.MinValue), Byte.MaxValue);
                output.G[k, i] = (Byte)Math.Min(Math.Max(Math.Round(intensityG / contrib[i].wsum), Byte.MinValue), Byte.MaxValue);
                output.B[k, i] = (Byte)Math.Min(Math.Max(Math.Round(intensityB / contrib[i].wsum), Byte.MinValue), Byte.MaxValue);
            }
